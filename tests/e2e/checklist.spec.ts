...
test("7+8. Аналитика: Оплаченные / Новые оплаченные / выручка ЮКасса+Наличные", async ({ page }) => {
  ...
  // UI показывает «Новые: {count}» — подсчёт первичных оплаченных
  expect(html, "Должен быть счётчик «Новые»").toMatch(/Новые:/i);
...
test("8. Поле ввода суммы при подтверждении CASH-записи", async ({ page }) => {
  ...
  // Найти вкладку «Наличные» (CASH-подтверждения)
  const cashTab = page.locator("text=/Наличные/i").first();
  if (await cashTab.isVisible({ timeout: 3000 }).catch(() => false)) {
    await cashTab.click();
    await page.waitForTimeout(1000);
  }
